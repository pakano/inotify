...
func (w *Watcher) Rename(oldpath string, newpath string) {
	w.mu.Lock()
	defer w.mu.Unlock()
	watch, found := w.watches[oldpath]
	if !found {
		return
	}
	w.watches[newpath] = watch
	w.paths[int(watch.wd)] = newpath
	delete(w.watches, oldpath)
}
